...
Bedrock Protocol thesis: self-describing embedded nodes (ESP32 smart node + ATtiny dumb nodes) speaking a
...
- Current plan (hardware affordances): `MILESTONE_E_AFFORDANCES_PLAN.md`
- ESP32 smart-node (ESP32forth): `firmware/esp32/bedrock.fs`
...
- Preferred CLI (Milestone C): `.venv/bin/python tools/terminal/bedrock_cli.py probe|load|define|snapshot ...`
  - Example load + persist: `.venv/bin/python tools/terminal/bedrock_cli.py load --persist --port /dev/cu.usbserial-0001`
  - Example snapshot: `.venv/bin/python tools/terminal/bedrock_cli.py snapshot create --out node.brsnap --port /dev/cu.usbserial-0001`
  - Record a session: `--transcript tools/terminal/transcripts/session.txt`
- TUI (early skeleton): `.venv/bin/python tools/terminal/bedrock_tui.py --port /dev/cu.usbserial-0001`
- Load/reload `bedrock.fs` (stable, prompt-independent):
  - Exit protocol mode (ok if already in REPL): `.venv/bin/python tools/terminal/bedrock_serial.py --port /dev/cu.usbserial-0001 --until end --line "repl"`
  - Send file line-by-line, waiting for `ok`: `.venv/bin/python tools/terminal/bedrock_serial.py --port /dev/cu.usbserial-0001 --until ok --file firmware/esp32/bedrock.fs`
  - Reload-safe: `bedrock.fs` begins with a `br-dev` + `forget` anchor (and forgets legacy `cd-dev`) to avoid dictionary-growth crashes.
...
- Smoke tests: `.venv/bin/python tools/terminal/bedrock_serial.py --port /dev/cu.usbserial-0001 --until end --line "?"` and `--line "history"`
...
- If the ESP32 “replays” a command endlessly on reconnect, reload the latest `firmware/esp32/bedrock.fs` (fixes `br-node` EOF/refill handling).
...
- Local cached refs (gitignored, not committed): `.bedrock/references/INDEX.txt` (see `.bedrock/references/ESP32forth.txt`, `.bedrock/references/ueforth_README.md`, etc.)
